				...
				for (cc = 0;  cc < n; cc += Tc)
				{
					for (r = rr;  r < std::min(rr+Tr-1, m-1); r+=2)
					{
						for (c = cc;  c < std::min(cc+Tc-3, n-3); c+=4)
						{
							...
							{
								//t = t + A[r*n+d] * B[d*n+c];
								Cr0c0 = Cr0c0 + A(r+0,d) * B(d,c+0);
								Cr0c1 = Cr0c1 + A(r+0,d) * B(d,c+1);
								Cr0c2 = Cr0c2 + A(r+0,d) * B(d,c+2);
								Cr0c3 = Cr0c3 + A(r+0,d) * B(d,c+3);

								Cr1c0 = Cr1c0 + A(r+1,d) * B(d,c+0);
								Cr1c1 = Cr1c1 + A(r+1,d) * B(d,c+1);
								Cr1c2 = Cr1c2 + A(r+1,d) * B(d,c+2);
								Cr1c3 = Cr1c3 + A(r+1,d) * B(d,c+3);
							}
